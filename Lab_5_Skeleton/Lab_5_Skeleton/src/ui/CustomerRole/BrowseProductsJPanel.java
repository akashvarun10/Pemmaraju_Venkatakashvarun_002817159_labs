...
try {
  ...
  int oldQuant = item.getQuantity();
    if (item.getProduct().getAvail() + oldQuant < quant) {
        JOptionPane.showMessageDialog(this, "Please check the product Availibity");
        return;
    }
...
if (salesPrice < product.getPrice()) {
    JOptionPane.showMessageDialog(this, "The price should be more than the base price set.");
    return;
}

OrderItem item = currentOrder.findProduct(product);

if (item == null) { // If the product is not already in the order
    if (product.getAvail() >= quant) {
        currentOrder.addNewOrderItem(product, salesPrice, quant);
        product.setAvail(product.getAvail() - quant);
    } else {
        JOptionPane.showMessageDialog(this, "Insufficient product availability.");
        return;
    }
} else { 
    int oldQuant = item.getQuantity();
    if (product.getAvail() + oldQuant < quant) {
        JOptionPane.showMessageDialog(this, "Insufficient product availability.");
        return;
    }
    item.getProduct().setAvail(item.getProduct().getAvail()+ oldQuant-quant);
